def test_interpolate_fbenm_torch_basic():
    ...
    result = interpolate_fbenm([r, p], nmove=5, output_file="tests/fbenm_ipopt.out",device="cpu")
...
def test_maxflux_torch():
    ...
    mxflx_fbenm = interpolate_fbenm(ref_images,correlated=True,device="cpu")
    coefs = mxflx_fbenm.coefs.copy()

    # set up a variational problem of the direct MaxFlux method
    mxflx = DirectMaxFlux(ref_images,coefs=coefs,nmove=3,update_teval=True,device="cpu")
