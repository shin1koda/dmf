...
Beyond these benchmarks, PyDMF has been adopted as a core component in higher-level computational workflows. Notably, it serves as the transition-state search engine in ColabReaction, a web-based application for transition-state searches [@karasawa2025colabrea]. PyDMF is also used as the underlying path-optimization engine in dmf-g16, a Gaussian wrapper that enables reliable double-ended transition-state searches from native Gaussian input files [@koda2026dmfg16]. These integrations demonstrate that PyDMF is not only a methodological implementation but also a practical and reusable software component that supports accessible and reliable transition-state searches.
